...
class Zoning_and_Demand_Manager extends IPSModule
{
    ...
    private function ensureCoilTempEvent(): void
    {
        ...
        if ($coilVarID <= 0 || !IPS_VariableExists($coilVarID)) {
            if ($eid > 0 && IPS_EventExists($eid)) IPS_SetEventActive($eid, false);
            return;
        }

        if ($eid <= 0 || !IPS_EventExists($eid)) {
            $eid = IPS_CreateEvent(0); // 0 = Triggered
            IPS_SetParent($eid, $this->InstanceID);
            IPS_SetName($eid, 'ZDM: CoilTemp Cutoff/Restart');
            $this->WriteAttributeInteger('CoilTempEventID', $eid);
        } else {
            $info = IPS_GetEvent($eid);
            if ((int)$info['EventType'] !== 0) { // must be Triggered
                IPS_DeleteEvent($eid);
        ...
        // One script for both directions: cutoff and restart
        $script = '/* Auto-generated by ZDM */
    $inst     = ' . $this->InstanceID . ';
    $var      = ' . $coilVarID . ';
    $val      = @GetValue($var);
    $cutoff   = (float)IPS_GetProperty($inst, "EmergencyShutdownTemp");
    $restart  = (float)IPS_GetProperty($inst, "EmergencyRestartTemp");

    if (is_numeric($val)) {
        $val = (float)$val;
        if ($val <= $cutoff) {
            @ZDM_HandleCoilBelowThreshold($inst, $val, $cutoff);
        } elseif ($val >= $restart) {
            @ZDM_HandleCoilAboveRestart($inst, $val, $restart);
        }
    }';
        IPS_SetEventScript($eid, $script);
        IPS_SetEventActive($eid, true);
        // IPS_SetHidden($eid, true); // optional
    }
